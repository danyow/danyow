...
> 本文从原根 README 移入，个人主页介绍现在单独维护。以下命令均在仓库根目录执行。
...
2026-09-16 增加已验证的独立阅读入口 `https://danyow.vercel.app/`。Vercel 连接发布仓库 `danyow/danyow.github.io` 的 `master`，读取同一个发布请求并独立构建当前源码；不代理或跳转主站。生成的 `gh-pages` 不再触发重复 Vercel 构建。详细配置与公网验收见[发布仓库说明](https://github.com/danyow/danyow.github.io#readme)。

README 维护不更新网站发布请求，不改日报任务和投递流程。它的独立维护规范见 [PROFILE.md](PROFILE.md)。
