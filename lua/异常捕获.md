...
这里重点看下 luaD\_pcall 的实现，这是pcall的核心处理函数：
...
看下 luaD\_rawrunprotected 函数的实现：
...
LUAI\_TRY 以及文章后面出现的LUAI\_THROW
...
当异常出现时，status 就会赋值为-1，即不等于 LUA\_OK
...
luaD\_throw则是调用 LUAI\_THROW，跳到前面catch的位置
...
在lua state中，nny记录了调用栈上不能被挂起的次数，定义在 lua\_State结构：
...
细心的同学会发现， luaD\_call是在pcall的执行过程调用到的， 为什么 L->nny会大于0？
...
那协程 lua\_newthread时也会调用 preinit\_thread ，但是两者对于pcall的实现却有不少的差异。

如下，协程创建时调用 preinit\_thread 预处理 协程：
